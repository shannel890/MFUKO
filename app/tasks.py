...
# from flask import current_app # Removed: app will be passed as an argument
from sqlalchemy import func, and_
import json

# Import for internationalization (lazy gettext)
...
from extension import db # Assuming 'db' is your SQLAlchemy instance

# Import your local 'api' module (corrected relative import)
from .api import mpesa_api # Ensure mpesa_api has a set_access_token method

# Assuming these utility functions exist and are properly implemented
...
def refresh_mpesa_token(app):
    ...
    It fetches a new access token from Safaricom.
    """
    # Use the passed 'app' instance to establish the application context
    with app.app_context():
        consumer_key = app.config.get('MPESA_CONSUMER_KEY')
        consumer_secret = app.config.get('MPESA_CONSUMER_SECRET')
        # Default M-Pesa OAuth URL
        auth_url = app.config.get('MPESA_AUTH_URL', 'https://api.safaricom.co.ke/oauth/v1/generate?grant_type=client_credentials')

        if not consumer_key or not consumer_secret:
            app.logger.error("MPESA_CONSUMER_KEY or MPESA_CONSUMER_SECRET not configured. Cannot refresh M-Pesa token.")
            return

        try:
            # Use requests's built-in auth parameter for Basic Authentication
            response = requests.get(auth_url, auth=requests.auth.HTTPBasicAuth(consumer_key, consumer_secret))
            response.raise_for_status() # Raise HTTPError for bad responses (4xx or 5xx)

            data = response.json()
            access_token = data.get('access_token')
            # Default expires_in to just under 1 hour (3599 seconds) if not provided
            expires_in = data.get('expires_in', 3599)

            if access_token:
                # Store the token globally in app.config
                # For multi-instance deployments, consider storing this in a shared cache (e.g., Redis) or DB.
                ...
                # If your mpesa_api client needs to be updated with the new token
            ...
            app.logger.error(f"Failed to refresh M-Pesa token due to network or HTTP error: {e}", exc_info=True)
        except json.JSONDecodeError:
            app.logger.error(f"Failed to decode M-Pesa token response JSON: {response.text}", exc_info=True)
        except Exception as e:
            app.logger.error(f"An unexpected error occurred during M-Pesa token refresh: {e}", exc_info=True)
